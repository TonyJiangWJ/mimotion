...
- 需要注意，为了避免出现问题，密钥请不要使用中文。请自行确保长度为16个字符。
- 当你账号过多或者需要自行登录时，请配置AES_KEY，以便开启自动保存。配置AES_KEY后，将自动将密钥加密保存到当前仓库，请一定保存好自己的AES密钥，不要泄露。
- 同时请确保配置正确的PAT密钥，否则将无法自动保存提交到仓库。
...
- 当配置了 `AES_KEY` 之后，因为每个人的仓库里面到会保存一份 `encrypted_tokens.data`，更新代码会会被覆盖。为了避免数据丢失，请提前备份，在更新完成后将它重新提交到仓库中。
